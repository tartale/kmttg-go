...
func (ts *TuringStream) GetCipherByte() byte {
	b := ts.cipherData[ts.cipherPos]
	ts.cipherPos++
	return b
}
...
func (ts *TuringStream) Reset(streamId, blockId int, turkey, turiv []byte) error {
	...
	ts.blockId = blockId
	ts.cipherPos = 0
	if err := ts.quickTuring.SetTuringKey(turkey, 20); err != nil {
		return err
	}
	if err := ts.quickTuring.SetTuringIV(turiv, 20); err != nil {
		return err
	}
	for i := range ts.cipherData {
		ts.cipherData[i] = 0
	}
	ts.cipherLen = ts.quickTuring.TuringGen(ts.cipherData)
	return nil
...
func (td *TuringDecoder) PrepareFrame(streamId, blockId int) *TuringStream {
	return td.PrepareFrameWithKey(td.key, streamId, blockId)
}

func (td *TuringDecoder) PrepareFrameWithKey(key []byte, streamId, blockId int) *TuringStream {
	stream, ok := td.streams[streamId]
	if !ok {
		stream = NewTuringStream(streamId, blockId)
		td.prepareFrameHelper(stream, key, streamId, blockId)
...
func (td *TuringDecoder) prepareFrameHelper(stream *TuringStream, key []byte, streamId, blockId int) {
	// Build a 20-byte key based on the stream-specific key material and the
	// stream/block identifier bytes appended at positions 16..19.
	keyCopy := make([]byte, 20)
	if len(key) >= 16 {
		copy(keyCopy, key[:16])
	} else {
		copy(keyCopy, key)
