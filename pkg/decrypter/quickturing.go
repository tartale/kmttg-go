...
// Direct port of net.straylightlabs.quickturing.QuickTuring.
// The S-box, Q-box, and multiplication table values are reused from Greg
// Rose's original Turing reference and the Java implementation.
...
type quickTuring struct {
	keyLen   int
	mixedKey [qtMaxKeyLen / 4]uint32
	reg      [qtRegLen]uint32
	s0, s1, s2, s3 [256]uint32
	buf    [qtMaxStreamLen]byte
	bufPos int
}

func (q *quickTuring) init(key, iv []byte) {
	q.setKey(key)
	q.setIV(iv)
	q.gen(q.buf[:])
	q.bufPos = 0
...
func (q *quickTuring) skip(n int) {
	for n > 0 {
		avail := qtMaxStreamLen - q.bufPos
		if n < avail {
			q.bufPos += n
			return
		}
...
func (q *quickTuring) decrypt(data []byte) {
	for i := range data {
		if q.bufPos >= qtMaxStreamLen {
			q.gen(q.buf[:])
			q.bufPos = 0
		}
		data[i] ^= q.buf[q.bufPos]
		q.bufPos++
	}
}

func (q *quickTuring) setKey(key []byte) {
	length := len(key)
	q.keyLen = 0
	for i := 0; i < length; i += 4 {
		q.mixedKey[q.keyLen] = qtFixedS(qtJoinWord(key, i))
		q.keyLen++
	}
	qtMixWords(q.mixedKey[:q.keyLen])

	// Build S-box lookup tables.
	for j := 0; j < 256; j++ {
		var w uint32
		k := uint32(j)
		for i := 0; i < q.keyLen; i++ {
			k = uint32(qtSBox[qtByte(q.mixedKey[i], 0)^byte(k)])
			w ^= qtRotL(qtQBox[k], uint(i))
		}
		q.s0[j] = (w & 0x00FFFFFF) | (k << 24)
	}
	for j := 0; j < 256; j++ {
		var w uint32
		k := uint32(j)
		for i := 0; i < q.keyLen; i++ {
			k = uint32(qtSBox[qtByte(q.mixedKey[i], 1)^byte(k)])
			w ^= qtRotL(qtQBox[k], uint(i+8))
		}
		q.s1[j] = (w & 0xFF00FFFF) | (k << 16)
	}
	for j := 0; j < 256; j++ {
		var w uint32
		k := uint32(j)
		for i := 0; i < q.keyLen; i++ {
			k = uint32(qtSBox[qtByte(q.mixedKey[i], 2)^byte(k)])
			w ^= qtRotL(qtQBox[k], uint(i+16))
		}
		q.s2[j] = (w & 0xFFFF00FF) | (k << 8)
	}
	for j := 0; j < 256; j++ {
		var w uint32
		k := uint32(j)
		for i := 0; i < q.keyLen; i++ {
			k = uint32(qtSBox[qtByte(q.mixedKey[i], 3)^byte(k)])
			w ^= qtRotL(qtQBox[k], uint(i+24))
		}
		q.s3[j] = (w & 0xFFFFFF00) | k
	}
}

func (q *quickTuring) setIV(iv []byte) {
	length := len(iv)
	i := 0
	for j := 0; j < length; j += 4 {
		q.reg[i] = qtFixedS(qtJoinWord(iv, j))
		i++
	}
	for j := 0; j < q.keyLen; j++ {
		q.reg[i] = q.mixedKey[j]
		i++
	}
	q.reg[i] = uint32((q.keyLen << 4) | (length >> 2) | 0x01020300)
	i++
	for j := 0; i < qtRegLen; i, j = i+1, j+1 {
		q.reg[i] = q.s(q.reg[j]+q.reg[i-1], 0)
	}
	qtMixWords(q.reg[:])
}

func (q *quickTuring) s(w uint32, r uint) uint32 {
	return q.s0[qtByte(w, (r)&0x3)] ^
		q.s1[qtByte(w, (1+r)&0x3)] ^
		q.s2[qtByte(w, (2+r)&0x3)] ^
		q.s3[qtByte(w, (3+r)&0x3)]
}

func (q *quickTuring) step(z int) {
	a := q.reg[(z+0)%qtRegLen]
	q.reg[z%qtRegLen] = q.reg[(z+15)%qtRegLen] ^ q.reg[(z+4)%qtRegLen] ^
		(a << 8) ^ qtMulTab[a>>24]
}

// gen writes 340 bytes of keystream into out (must be >= 340 bytes).
func (q *quickTuring) gen(out []byte) {
	zs := [17]int{0, 5, 10, 15, 3, 8, 13, 1, 6, 11, 16, 4, 9, 14, 2, 7, 12}
	for i, z := range zs {
		q.genRound(z, out[i*qtRoundOutLen:])
	}
}

func (q *quickTuring) genRound(z int, out []byte) {
	q.step(z)

	a := q.reg[(z+1+16)%qtRegLen]
	b := q.reg[(z+1+13)%qtRegLen]
	c := q.reg[(z+1+6)%qtRegLen]
	d := q.reg[(z+1+1)%qtRegLen]
	e := q.reg[(z+1+0)%qtRegLen]
	...
	a += q.reg[(z+4+14)%qtRegLen]
	b += q.reg[(z+4+12)%qtRegLen]
	c += q.reg[(z+4+8)%qtRegLen]
	d += q.reg[(z+4+1)%qtRegLen]
	e += q.reg[(z+4+0)%qtRegLen]

	qtWordToBytes(a, out[0:])
	qtWordToBytes(b, out[4:])
	qtWordToBytes(c, out[8:])
	qtWordToBytes(d, out[12:])
	qtWordToBytes(e, out[16:])

	q.step(z + 4)
}

func qtFixedS(w uint32) uint32 {
	for i := uint(0); i < 4; i++ {
		shift := i * 8
		octet := uint32(qtSBox[qtByte(w, i)])
		w = ((w ^ qtRotL(qtQBox[octet], shift)) & qtRotR(0x00FFFFFF, shift)) |
			(octet << (24 - shift))
	}
	return w
}

func qtMixWords(w []uint32) {
	var sum uint32
	for i := 0; i < len(w)-1; i++ {
		sum += w[i]
	}
...
func qtJoinWord(b []byte, off int) uint32 {
	return (uint32(b[off]) << 24) | (uint32(b[off+1]) << 16) |
		(uint32(b[off+2]) << 8) | uint32(b[off+3])
}

func qtWordToBytes(w uint32, b []byte) {
	b[0] = byte(w >> 24)
	b[1] = byte(w >> 16)
	b[2] = byte(w >> 8)
	b[3] = byte(w)
...
func qtByte(w uint32, i uint) byte { return byte(w >> (24 - 8*i)) }

func qtRotL(w uint32, n uint) uint32 {
	n &= 31
	if n == 0 {
		return w
	}
...
func qtRotR(w uint32, n uint) uint32 {
	n &= 31
	if n == 0 {
		return w
	}
...
// S-box from Greg Rose's Turing reference implementation.
var qtSBox = [256]byte{
...
// Q-box generated by Millan et al. at Queensland University of Technology.
var qtQBox = [256]uint32{
